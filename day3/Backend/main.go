...
// 	var newCar Car
// 	if err:= c.ShouldBlindJSON(&newCar);err!=nil{
...
// 			saveCars(cars)
// 			c.JSON(http.StatusOK, gin.H{"message": "Car parking details fetched successfully!"})
// 			return
// 		}
// 	}
// 	c.JSON(http.StatusNotFound, gin.H{"error": "Car details not found"})
...
// 	r.PUT("/cars",updateCars)
// 	r.DELETE("/cars",deleteCars)
...
func deleteCar(c *gin.Context) {
	...
	for i, car := range cars {
		if car.ID == id {
			cars = append(cars[:i], cars[i+1:]...)
			saveCars(cars)
			c.JSON(http.StatusOK, gin.H{"message": "Car parking details deleted successfully!"})
			return
		}
	}
	c.JSON(http.StatusNotFound, gin.H{"error": "Car details not found"})
